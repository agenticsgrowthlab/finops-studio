...
function calcArchScore(services, guardrails) {
  let score = 100
  const cachingPct = services.length > 0 ? services.filter(s => s.caching_enabled).length / services.length : 1
...
router.get('/', async (req, res) => {
  try {
    ...
    const projects = await pool.query(`SELECT * FROM projects WHERE org_id = $1 ORDER BY updated_at DESC`, [orgId])
    const enriched = await Promise.all(projects.rows.map(async (p) => {
      ...
      return {
        ...p,
        cost_score: calcCostScore(services.rows, p.budget_annual),
        arch_score: calcArchScore(services.rows, guardrails.rows),
        risk_level: calcRiskLevel(guardrails.rows, alerts.rows),
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const [services, guardrails, alerts, snapshots, recommendations, decisions, budgets, estimates] = await Promise.all([
    ...
    res.json({
      ...
      data: {
        ...project,
        cost_score: calcCostScore(services.rows, project.budget_annual),
        arch_score: calcArchScore(services.rows, guardrails.rows),
        risk_level: calcRiskLevel(guardrails.rows, alerts.rows),
        services: services.rows, guardrails: guardrails.rows, alerts: alerts.rows,
        snapshots: snapshots.rows, recommendations: recommendations.rows,
        decisions: decisions.rows, budgets: budgets.rows,
...
router.post('/', async (req, res) => {
  try {
    ...
    const r = await pool.query(`
      ...
      VALUES ($1,$2,$3,$4,$5,$6,85,'B','low') RETURNING *
    `, [orgId, name, type, status, description || null, budget_annual || null])
    if (budget_annual) {
      await pool.query(`INSERT INTO budgets (project_id,year,annual_target) VALUES ($1,$2,$3) ON CONFLICT (project_id,year) DO NOTHING`,
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const r = await pool.query(`
      ...
      budget_annual=COALESCE($3,budget_annual), status=COALESCE($4,status), updated_at=NOW()
